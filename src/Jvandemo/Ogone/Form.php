...
class Form
{
    ...
    /**
     * Array of parameters (any values specified here will be used as default, but can be overridden by constructor
     * or addParam())
     *
     * @var array
     */
    ...
    {
        if ($key === null || (!array_key_exists($key, $this->params))) {
            return '';
        }
    ...
    {
        $html = '';
        $html .= '<form method="' . $this->config['formMethod'] . '" id="' .
            $this->config['formId'] . '" action="' .
            $this->config['formAction'] . '" name="' .
            $this->config['formName'] . '" class="' .
            $this->config['formClass'] . '">' . PHP_EOL;

        foreach ($this->params as $key => $value) {
            if ($value != '') {
                $html .= '<input type="hidden" name="' . $key . '" value="' .
                    $value . '" class="' . $this->config['formElementClass'] .
                    '" />' . PHP_EOL;
            }
        }
